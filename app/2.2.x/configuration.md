...
**pg_ssl** | Toggles client-server TLS connections between Kong and PostgreSQL. Since PostgreSQL uses the same port for TLS and non-TLS this is only a hint. If the server does not support TLS the established connection will be a plain one. | `off`
